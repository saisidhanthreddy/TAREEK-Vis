...
MainWindow::buildActivityMarkers(uint32_t personId, int tripIndex) const {
    ...
    if (!trips) return markers;

    // Activity position = to-node of the trip's endpoint link
    auto linkToNodePos = [&](uint32_t linkId, float& x, float& y) -> bool {
        if (linkId == TRIP_NO_LINK) return false;
        const auto* link = networkIndex_->getLink(linkId);
        if (!link) return false;
        const auto* node = networkIndex_->getNode(link->toNode);
    ...
    for (int i = 0; i < static_cast<int>(trips->size()); ++i) {
        if (tripIndex >= 0 && i != tripIndex) continue;
        const auto& trip = (*trips)[i];

        QString fromAct = vehicleIndex_->actTypeString(trip.fromActTypeId);
        QString toAct = vehicleIndex_->actTypeString(trip.toActTypeId);

        float x, y;
        if (!fromAct.isEmpty() && linkToNodePos(trip.fromLinkId, x, y)) {
            markers.push_back({x, y, panelstyle::actIcon(fromAct), fromAct});
        }
        if (!toAct.isEmpty() && linkToNodePos(trip.toLinkId, x, y)) {
            markers.push_back({x, y, panelstyle::actIcon(toAct), toAct});
        }
    }

    // Dedupe: the same activity shows up as trip N's destination and trip
    // N+1's origin at the same node
